...
{
    XiaoxhPhotoGroupItem* item = [self.class new];
    return item;
}
@end

@interface XiaoxhPhotoGroupCell : UIScrollView <UIScrollViewDelegate>
...
@property (nonatomic, strong) XiaoxhPhotoGroupItem* item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation XiaoxhPhotoGroupCell
...
- (void)setItem:(XiaoxhPhotoGroupItem*)item
{
    if (_item == item)
        return;
    _item = item;
...
@interface XiaoxhPhotoGroupView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) UILabel* pagerLab;
...
@implementation XiaoxhPhotoGroupView
...
- (instancetype)initWithGroupItems:(NSArray*)groupItems
{
    ...
    _pagerLab = [[UILabel alloc] init];
    _pagerLab.width = self.width - 36;
    _pagerLab.height = 15;
    _pagerLab.font = [UIFont systemFontOfSize:14];
    _pagerLab.textColor = [UIColor whiteColor];
    _pagerLab.center = CGPointMake(self.width / 2, self.height - 18);
    _pagerLab.textAlignment = NSTextAlignmentCenter;
    
    [self addSubview:_background];
    [self addSubview:_blurBackground];
    [self addSubview:_contentView];
    [_contentView addSubview:_scrollView];
    [_contentView addSubview:_pagerLab];
                  ...
                  completion:(void (^)(void))completion
{
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((XiaoxhPhotoGroupItem*)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    self.pagerLab.alpha = 0;
    self.pagerLab.text = [NSString stringWithFormat:@"%ld/%ld", page + 1, self.groupItems.count];
    ...
    XiaoxhPhotoGroupCell* cell = [self cellForPage:self.currentPage];
    XiaoxhPhotoGroupItem* item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion
{
    ...
    XiaoxhPhotoGroupCell* cell = [self cellForPage:currentPage];
    XiaoxhPhotoGroupItem* item = _groupItems[currentPage];
...
{
    [_cells enumerateObjectsUsingBlock:^(XiaoxhPhotoGroupCell* cell, NSUInteger idx, BOOL* stop) {
...
- (void)scrollViewDidScroll:(UIScrollView*)scrollView
{
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            XiaoxhPhotoGroupCell* cell = [self cellForPage:i];
            if (!cell) {
                XiaoxhPhotoGroupCell* cell = [self dequeueReusableCell];
                cell.page = i;
    ...
    _pagerLab.text = [NSString stringWithFormat:@"%ld/%ld", intPage + 1, self.groupItems.count];
...
{
    for (XiaoxhPhotoGroupCell* cell in _cells) {
        if (cell.superview) {
...
- (XiaoxhPhotoGroupCell*)dequeueReusableCell
{
    XiaoxhPhotoGroupCell* cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (XiaoxhPhotoGroupCell*)cellForPage:(NSInteger)page
{
    for (XiaoxhPhotoGroupCell* cell in _cells) {
        if (cell.page == page) {
            return cell;
...
- (void)doubleTap:(UITapGestureRecognizer*)g
{
    if (!_isPresented)
        return;
    XiaoxhPhotoGroupCell* tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
